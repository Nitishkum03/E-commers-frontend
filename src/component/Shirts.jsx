...
export default function Shirts() {
  ...
  // Filter for shirts and t-shirts (case-insensitive)
  const shirtProducts = products.filter(p => /shirt|t-shirt/i.test(p.name));
  // Pick 6 random related products from filtered list
  const related = shirtProducts.sort(() => 0.5 - Math.random()).slice(0, 6);
  ...
  return (
    <div className="min-h-screen px-4 py-8 flex flex-col items-center">
      <div className="grid grid-cols-1 sm:grid-cols-2 sm:-p-2 md:grid-cols-3 gap-4 max-w-[1500px] max-h-full mx-auto w-full">
        ...
        {shirtProducts[0] && (
          <div className="md:col-span-2 bg-black rounded-xl p-2 flex justify-center items-center hover:border-green-500 border border-transparent ">
            <Link to={`/product/${shirtProducts[0]._id}`} className="w-full">
              <ProductCard product={shirtProducts[0]} />
            </Link>
          </div>
        )}
        ...
        <div className="flex flex-col gap-4 ">
          {shirtProducts[1] && (
            <div className="bg-black rounded-xl p-4 flex justify-center items-center hover:border-green-500 border border-transparent">
              <Link to={`/product/${shirtProducts[1]._id}`} className="w-full">
                <ProductCard product={shirtProducts[1]} />
              </Link>
            </div>
          )}
          {shirtProducts[2] && (
            <div className="bg-black rounded-xl p-4 flex justify-center items-center hover:border-green-500 border border-transparent">
              <Link to={`/product/${shirtProducts[2]._id}`} className="w-full">
                <ProductCard product={shirtProducts[2]} />
              </Link>
            </div>
          )}
        </div>
      ...
      <div className="w-full max-w-[1500px] mt-10 flex-wrap">
        <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 "> Similar Shirts</h2>
