...
export default function Stickers() {
  ...
  // Filter for stickers (case-insensitive)
  const stickerProducts = products.filter(p => /sticker/i.test(p.name));
  // Pick 6 random related products from filtered list
  const related = stickerProducts.sort(() => 0.5 - Math.random()).slice(0, 6);
  ...
  return (
    <div className="min-h-screen px-4 py-8 flex flex-col items-center">
      <div className="grid grid-cols-1 sm:grid-cols-2 sm:-p-2 md:grid-cols-3 gap-4 max-w-[1500px] max-h-full mx-auto w-full">
        ...
        {stickerProducts[0] && (
          <div className="md:col-span-2 bg-black rounded-xl p-2 flex justify-center items-center hover:border-green-500 border border-transparent ">
            <Link to={`/product/${stickerProducts[0]._id}`} className="w-full">
              <ProductCard product={stickerProducts[0]} />
            </Link>
          </div>
        )}
        ...
        <div className="flex flex-col gap-4 ">
          {stickerProducts[1] && (
            <div className="bg-black rounded-xl p-4 flex justify-center items-center hover:border-green-500 border border-transparent">
              <Link to={`/product/${stickerProducts[1]._id}`} className="w-full">
                <ProductCard product={stickerProducts[1]} />
              </Link>
            </div>
          )}
          {stickerProducts[2] && (
            <div className="bg-black rounded-xl p-4 flex justify-center items-center hover:border-green-500 border border-transparent">
              <Link to={`/product/${stickerProducts[2]._id}`} className="w-full">
                <ProductCard product={stickerProducts[2]} />
              </Link>
            </div>
          )}
        </div>
      ...
      <div className="w-full max-w-[1500px] mt-10 flex-wrap">
        <h2 className="text-2xl md:text-3xl font-bold text-white mb-6 "> Similar Stickers</h2>
